package dev.lumareth.aqueensdecor.world.dimension;
...
public class UnderworldPortalShape {
    ...
    private static final BlockBehaviour.StatePredicate FRAME = (blockState, blockGetter, blockPos) -> blockState.is(Blocks.REINFORCED_DEEPSLATE);
    ...
    private UnderworldPortalShape(Direction.Axis axis, int i, Direction direction, BlockPos blockPos, int j, int k) {
        this.axis = axis;
    ...
    public static Optional<UnderworldPortalShape> findEmptyPortalShape(LevelAccessor levelAccessor, BlockPos blockPos, Direction.Axis axis) {
        return findPortalShape(levelAccessor, blockPos, modPortalShape -> modPortalShape.isValid() && modPortalShape.numPortalBlocks == 0, axis);
    }

    public static Optional<UnderworldPortalShape> findPortalShape(LevelAccessor levelAccessor, BlockPos blockPos, Predicate<UnderworldPortalShape> predicate, Direction.Axis axis) {
        Optional<UnderworldPortalShape> optional = Optional.of(findAnyShape(levelAccessor, blockPos, axis)).filter(predicate);
    ...
    public static UnderworldPortalShape findAnyShape(BlockGetter blockGetter, BlockPos blockPos, Direction.Axis axis) {
        ...
        if (i == 0) {
            return new UnderworldPortalShape(axis, 0, direction, blockPos2, 0, 0);
        }

        MutableInt mutableInt = new MutableInt();
        int j = calculateHeight(blockGetter, blockPos2, direction, i, mutableInt);
        return new UnderworldPortalShape(axis, mutableInt.getValue(), direction, blockPos2, i, j);
    }
    @Nullable
    ...
    public static boolean tryLight(LevelAccessor levelAccessor, BlockPos blockPos) {
        Optional<UnderworldPortalShape> optional = findEmptyPortalShape(levelAccessor, blockPos, Direction.Axis.X);
        if (optional.isPresent()) {
    ...
    public void createPortalBlocks(LevelAccessor levelAccessor) {
        BlockState blockState = ModBlocks.UNDERWORLD_PORTAL.defaultBlockState().setValue(NetherPortalBlock.AXIS, this.axis);
